...
export default class StaticTabbar extends React.PureComponent<StaticTabbarProps> {
  ...
  onPress = (index: number, route: string) => {
      ...
      <View style={styles.container}>
        {
          tabs.map((tab, key) => {
            ...
            return (
              <React.Fragment {...{ key }}>
                <TouchableWithoutFeedback onPress={() => onPress(key, tab.route)}>
                  <Animated.View style={[styles.tab, { opacity }]}>
                    <Feather name={tab.name} color={currIndex == key ? 'rgba(33,150,243 ,1)' : '#212121'} size={20} />
                  </Animated.View>
